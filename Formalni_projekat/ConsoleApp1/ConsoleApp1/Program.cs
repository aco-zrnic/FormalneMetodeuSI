...
namespace IzmjenaPrograma
{
    ...
    class TextValid
    {
        ...
        public string ToUpper(string str)
        {
            ...
            foreach (Match m in matches)
            {
               ...
               string pom_str = m.ToString();
                ...
                Regex rgx = new Regex(@"\<all_caps\>.+?\<\/all_caps\>");
                str = rgx.Replace(str, pom_str, 1);
            }
            return str;
        }
        ...
        public int InlineReplace(string stringOfAdvencInlineStrings,string oprtAritm, string oprtAritamAndString,ref string str,ArrayList arrayList, string oprtInlineBase)
        {
            int decissonNumber = 0;
            MatchCollection matchfirAdvencInline = Regex.Matches(stringOfAdvencInlineStrings, oprtAritm); //za provjeru aritmetickih variabli inline
            string collectorOfFirstRegexAdvencInline = "", collectorOfSecondRegexAdvencInline = "";
            foreach (Match mfir in matchfirAdvencInline)
            {
                collectorOfFirstRegexAdvencInline += mfir.ToString();
            }
            MatchCollection matchsecAdvencInline = Regex.Matches(stringOfAdvencInlineStrings, oprtAritamAndString); //za provjeru slozenog izraza inline
            foreach (Match m in matchsecAdvencInline)
            {
                collectorOfSecondRegexAdvencInline += m.ToString();
            }

            if (stringOfAdvencInlineStrings.Equals(collectorOfFirstRegexAdvencInline) && stringOfAdvencInlineStrings.Length >= 1)
            {
                decissonNumber= Convert.ToInt32(ArithmeticVarCheck(stringOfAdvencInlineStrings, matchfirAdvencInline, arrayList, matchsecAdvencInline));
                if (decissonNumber != 0) return decissonNumber;

            }
            else if (stringOfAdvencInlineStrings.Equals(collectorOfSecondRegexAdvencInline) && stringOfAdvencInlineStrings.Length >= 1)
            {
                decissonNumber= Convert.ToInt32(ComplexVarCheck(stringOfAdvencInlineStrings, matchsecAdvencInline, arrayList));
                if (decissonNumber != 0) return decissonNumber;
            }
            Regex regex = new Regex(oprtInlineBase);

            str = regex.Replace(str, "", 1); //da izbacimo {} i da mozemo ispisati bez toga
            return 0;
        }
        public string VarReplaceWithVal(string str,ArrayList arrayList)
        {
        ...
        public bool ArithmeticVarCheck(string str, MatchCollection matchfir, ArrayList arrayList, MatchCollection matchsec)
        {
            ...
            foreach (Match m in matchfir)
            {
                foreach (Variable var in arrayList)
                {
                    ...
                    {
                        return ComplexVarCheck(str, matchsec, arrayList);
                    }
                }
            }
            ...
            for (int i = 0; i < matchfir.Count; i++)  //foreach (Match m in matchsec)
            {
                ...
                else if (matchfir[0].Groups[1].Value != "")
                {
                    ...
                    if ((i == 1 && matchfir[i].Groups[2].Value != "") || (i == (matchfir.Count - 1) && matchfir[i].Groups[2].Value != ""))  //provjera jel prvi znak + pa onda string sledi ili zadnji znak +
                    {
                        ...
                        {
                            if (character == '+' || character == ' ' || character == 9 || character == '-') //moze bez uslova za " " i 9,kasnije mozda izbacim,PROVJERITI JOS SLUCAJEVA!
                                counterOfPlusAndSpace++;
                        }
                        if (counterOfPlusAndSpace == matchfir[i].Groups[2].Length)
                        {
                            Console.WriteLine("Ne moze ici + prvo pa onda[var] ili sam + na kraju->{0}", str);
                            return true;
                        }
                    ...
                    {
                        if (counterOfPlus > 0 && (matchfir[i].Groups[2].Value.Contains("+") || matchfir[i].Groups[2].Value.Contains("-")|| matchfir[i].Groups[2].Value.Contains("*") || matchfir[i].Groups[2].Value.Contains("/")))
                            counterOfPlus--;                  
                        varOb.strings += matchfir[i].Groups[2].Value;
                         
                    }
                    counterOfPlusAndSpace = 0;
                    if (counterOfPlus >= 2)
                    {
                        Console.WriteLine("Fali znak izmedju izmedju->{0}", str);
                        return true;
                    }
                }
                else
                ...
                else if (matchsec[0].Groups[1].Value != "")
                {
                    ...
                    else if (Regex.IsMatch(matchsec[i].Value, @"[\d\(\)\+\-\/\*\s]+"))
                    {
                        ...
                        if (counterOfPlusAndSpace != matchsec[i].Groups[3].Length)
                        {
                            ...
                            if(((matchsec[i].Groups[3].Value.Length-1)!=matchsec[i].Groups[3].Value.LastIndexOf('+')) && i < (matchsec.Count - 1))
                            {

                                Console.WriteLine("Pogresno definisan aritmeticki izraz,fali PLUS na kraju aritemtickog izraza->{0}", str);
                                return true;
                            }
                            else
                            {
        ...
        public void Inline2018OneFile(string nameofFile)
        {
            TextValid textValid = new TextValid();
            int decissonNumb = 0;
            int decissonForOutput = 0;
            try
            {
                using (StreamReader reader = new StreamReader(nameofFile)) //whe 0-element
                {
                    string str = "";
                    string oprtAritm = @"\[([\w\d\.\$\#_]+)\]\s*\=\s*|([\d\(\)\+\-\/\*\s]+)|\[([\w\d\.\$\#_]+)\]";
                    string oprtAritamAndString = @"\[([\w\d\.\$\#_]+)\]\s*\=\s*|" + "\u0022" + @"([\w\d\s\.\$\#_]+)" + "\u0022" + @"|([\d\(\)\+\-\/\*\s]+)|\[([\w\d\.\$\#_]+)\]"; //ovde je kao trebao na kraju ici zagrada )
                    ...
                    while ((str = reader.ReadLine()) != null)
                    {
                        if (str != "")
                        {
                            ...
                            {
                                decissonNumb = Convert.ToInt32(textValid.ArithmeticVarCheck(str, matchfir, arrayList, matchsec)); //saljem matchsec jer matchfir ima drukcije definisane grupe,i ako matche-uje sve
                                if (decissonNumb != 0) break;
                                decissonForOutput = 1;
                            }
                            ...
                            string oprtInlineAdvenc = @"\@\{\s*|(\[([\w\d\.\$\#_]+)\]\s*\=\s*|" + "\u0022" + @"([\w\d\s\.\$\#_]+)" + "\u0022" + @"|([\d\(\)\+\-\/\*\s])+|\[([\w\d\.\$\#_]+)\])|\s*\}";
                            ...
                            if (stringOfAdvencInlineStrings.Length > 0)
                            {
                                stringOfAdvencInlineStrings = "";
                                foreach (Match m in matchInlineAvanc)
                                {
                                    if (m.Value.Contains("@{"))
                                        des = 1;
                                    ...
                                    {
                                        des = 0;
                                        decissonNumb = textValid.InlineReplace(stringOfAdvencInlineStrings, oprtAritm, oprtAritamAndString, ref str, arrayList, oprtInlineBase);
                                        if (decissonNumb != 0)
                                            break;
                                        stringOfAdvencInlineStrings = "";
                                    }
                            ...
                            if (decissonNumb != 0)
                                break;
                            str = textValid.VarReplaceWithVal(str, arrayList); //zamjena variabli sa njihovim vrijednostima
                            str = textValid.ToUpper(str); //converts lowCase to UperCase and removes <all_caps> and </all_caps>
                            if (Regex.IsMatch(str, @"include\s*(.+\.txt)")) //provjerava jel postoji include file
                            {
                                Match matchForInclude = Regex.Match(str, @"include\s*(.+\.txt)");
                                if (str.Equals(matchForInclude.ToString()))
                                {
                                    Inline2018OneFile(matchForInclude.Groups[1].Value);
                                    decissonForOutput = 1;
                                }
                                else
                                {
                                    Console.WriteLine("Nepravilna linija koda kod include->{0}", str);
                                    break;
                                }
                            }
                            if (decissonForOutput == 0)
                            {
                                Match matchstring = Regex.Match(str, @"[\w\d\.\#\$\s\-_]+"); //da li sad string sadrzi samo dozvoljene karakter za <tekst>
                                if (!str.Equals(matchstring.ToString()))
                                {
                                    Console.WriteLine("Ne dozvoljen tekst se pojavio ->{0}", str);
                                    break;
                                }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            textValid.Inline2018OneFile(nameofFile);
            Console.ReadLine();
        }
    }
}
